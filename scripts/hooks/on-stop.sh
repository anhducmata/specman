...
# current-session pointer is in the project's .claude/ dir (where this hook fires)
PROJECT_DIR="$(pwd)"
...
# Session file missing — clear state and exit
if [[ ! -f "$SESSION_PATH" ]]; then
  rm -f "$CURRENT_SESSION_FILE"
  exit 0
fi

# Already completed — clear state and exit
...
# Append to session-index.md if not already present
